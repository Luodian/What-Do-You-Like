...
public class FetchData {
    ...
    public ArrayList<BookItem> SearchBooksByAuthorNameUseLike(String name)
    {
        ArrayList<BookItem> ret = null;
        ret = new ArrayList<>();
        connector.ConnectDataBase();
        PreparedStatement pst = null;
        ResultSet rst = null;
        String SEARCH_SQL = "SELECT ISBN,Author,AuthorID,Name,price,ImageUrl,PublishDate,Publisher,Comment FROM Book where Author LIKE ?";
        try
        {
            pst = connector.connect.prepareStatement(SEARCH_SQL);
            pst.setString(1,"%" + name + "%");
